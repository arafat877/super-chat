...
const Conversations = ({ currentUser }) => {
  ...
  const { isFetching, error, dispatch } = useContext(
    ConversationContext
  );

  useEffect(() => {
     getPrivateConversations(currentUser._id,setPrivateConversations,dispatch).then(conversations=>console.log(conversations))
