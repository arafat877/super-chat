...
const Register = () => {
  ...
  const handleConfirmPassword = (event) => {
    if (event.target.value !== form.password) {
      setValidationErrors({ ...validationErrors, passwordAgain: "Passwords do not match!" });
    } else {
  ...
  return (
    <div className="registerContainer vh-100 d-flex">
      ...
      <Link to="login"><button className="btn btn-success loginButton">Allready member</button></Link>
    </div>
  );
};
